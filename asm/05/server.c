...
int main(int argc, char *argv[]) {
  ...
  while (1) {
     ...
     * incoming client request, the three way TCP handshake* is complete, the
     * function accept() wakes up and returns the socket descriptor representing
     * the client socket.
     */
    connfd = accept(listenfd, (struct sockaddr *)NULL, NULL);

    /* As soon as server gets a request from client, it prepares the date and
     * time and writes on the client socket through the descriptor returned by
     * accept()
     */
    ticks = time(NULL);
